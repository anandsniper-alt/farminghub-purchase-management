...
try{
 ...
 for(const mode of ['server','review']){
  ...
  const initial=await read(),a=initial.orders[0],b=initial.orders[12];check(mode+': stable serial column visible',await page.getByRole('button',{name:'Sort by S.No.',exact:true}).count()===1);
