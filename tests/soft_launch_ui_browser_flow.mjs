...
const make=(id,change={})=>({...structuredClone(template),id,number:'QA-REGRESSION-'+id,serialNumber:1,buyerId:'u-manager',shipments:[],tasks:[{id:'task-'+id,title:'Supplier follow-up',ownerId:'u-manager',due:'2026-09-19'}],...change});
...
try{
 ...
 for(const mode of ['server','review']){
  ...
  await page.goto(root+'#/orders');await page.getByRole('button',{name:'Create purchase order',exact:true}).click();await page.locator('[name=vendorId]').selectOption(base.vendorId);await page.locator('[name=base-0]').selectOption(base.id);await page.locator('[name=qty-0-GJ]').fill('2');await page.locator('[name=qty-0-KD]').fill('3');await page.locator('[name=baseprice-0]').fill('10');await page.locator('[name=planningTat]').fill('90');await page.locator('[name=routeId]').selectOption({index:1});
