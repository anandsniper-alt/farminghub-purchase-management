// Preview-only progressive presentation; no app state or permission changes.
const body=document.body,strip=document.querySelector('.theme-lab'),reduced=matchMedia('(prefers-reduced-motion: reduce)');
let theme=localStorage.getItem('fh-theme-lab-style')||'minimal',guides=false,lastView='',lastMain=null,lastModal=null,scheduled=false;
...
function replayState(){const replay=strip.querySelector('.theme-replay');replay.disabled=theme==='current'||reduced.matches;replay.title=reduced.matches?'Animations disabled by your reduced-motion preference':'Replay the page entrance';}
...
function apply(){headings();body.dataset.previewTheme=theme;body.dataset.previewGuides=guides?'on':'off';for(const b of strip.querySelectorAll('[data-theme]'))b.setAttribute('aria-pressed',String(b.dataset.theme===theme));const help=strip.querySelector('.theme-guide');help.setAttribute('aria-pressed',String(guides));help.textContent=guides?'Hide page guides':'Show page guides';help.disabled=theme==='current';for(const d of document.querySelectorAll('.preview-workflow,.preview-guidance,.preview-actions'))d.open=theme==='current'||(guides&&d.classList.contains('preview-guidance'));}
function enhance(){scheduled=false;const main=document.querySelector('main.content'),modal=document.querySelector('#modal-root .modal');if(main&&main!==lastMain){lastMain=main;headings();const track=main.querySelector('.progress-track');if(track&&!track.closest('.preview-workflow')){const wrapper=document.createElement('details');wrapper.className='preview-workflow';const summary=document.createElement('summary');summary.textContent='View all workflow stages';track.before(wrapper);wrapper.append(summary,track);wrapper.open=theme==='current';}
...
strip.addEventListener('click',event=>{const toggle=event.target.closest('[data-theme]');if(toggle){stopMotion();theme=toggle.dataset.theme;localStorage.setItem('fh-theme-lab-style',theme);apply();replayState();animate(document.querySelector('main.content'));}else if(event.target.closest('.theme-replay')){animate(document.querySelector('main.content'));}else if(event.target.closest('.theme-guide')){guides=!guides;apply();}});
