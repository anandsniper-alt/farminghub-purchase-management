...
const fieldHelp={number:'Copy the document number exactly. The Farming Hub software reference is assigned separately. Do not reuse an old number.',vendorId:'Choose the supplier named on the quote. Check the supplier name and code before adding items.',currency:'Choose the currency the supplier will invoice. RMB is shown as CNY in stored records.',priceListCurrency:'Choose the currency actually used in the supplier price sheet. It may differ from the invoice currency.',priceListFxRate:'Enter the supplier-agreed RMB for 1 USD. Use the quote for this PO; do not guess the rate.',priceListFxDate:'Use the date on which the supplier agreed this exchange rate.',reference:'Copy the unique bank transaction reference from the bank proof. Record a payment only after the bank made it.',inrRate:'Enter the Indian bank rate for 1 unit of the remittance currency. Check the direction of conversion.',invoiceNumber:'Copy the commercial invoice number from the supplier document. It must not belong to another order or shipment of this supplier.',invoiceDate:'Use the commercial invoice date printed on the supplier document.',realizedAmount:'Enter what the supplier actually received, in the displayed currency. Do not copy the amount sent unless they are confirmed equal.'};
