...
const routes={
 home:[step('.module-cards','Choose a module','Open Order Management for purchasing, or Vendor Management System for vendor activity.')],
 'order-management':[step('.division-cards','Choose a division','LAE Import contains the current purchasing tools. LAE Domestic, Utility Division and Implements Division are pending development.')],
 vms:[step('.page-head','Vendor management','VMS and Purchase use the same supplier records and login. Open a supplier to review its contacts, sourcing activity and linked orders.'),step('.vms-nav','1. Choose your work','Vendor Management System has its own navigation. Use Vendors and Vendor Follow-up for daily work, Analytics and Sourcing Risk for reports, and the catalogue screens for setup.'),step('#vms-search','2. Find a supplier','Search the supplier name, stable vendor reference, contact or city. Sourcing-stage filters do not change purchase eligibility.'),step('[data-action=vms-profile]','3. Maintain the profile','Purchase Managers can maintain contacts, sourcing tags and ownership. Purchase defaults remain in the shared Vendor master.'),step('[data-action=vms-interaction]','4. Record contact and the next step','Use Record visit for a visit, or Add interaction for other contact. Enter notes or evidence. Set a next follow-up only when another action is needed. Purchase order tasks remain separate.'),step('[data-action=vms-followup]','5. Update the follow-up','Complete or reschedule the follow-up with notes. Every scheduled action remains visible until completed. Completing one action does not hide other open work. Visit history retains the outcome.'),step('[data-action=vms-evaluation]','6. Evaluate deliberately','Scores are weighted recommendations. A partial score uses only rated criteria, and does not approve a vendor or purchase order.')],
...
const fieldHelp={number:'Copy the supplier document number exactly. PO numbers are automatic. Supplier PI and commercial invoice numbers must be unique within that supplier.',vendorId:'Choose the supplier named on the quote. Check the supplier name and code before adding items.',currency:'Choose the currency the supplier will invoice. RMB is shown as CNY in stored records.',priceListCurrency:'Choose the currency actually used in the supplier price sheet. It may differ from the invoice currency.',priceListFxRate:'Enter the supplier-agreed RMB for 1 USD. Use the quote for this PO; do not guess the rate.',priceListFxDate:'Use the date on which the supplier agreed this exchange rate.',reference:'Copy the unique bank transaction reference from the bank proof. Record a payment only after the bank made it.',inrRate:'Enter the Indian bank rate for 1 unit of the remittance currency. Check the direction of conversion.',invoiceNumber:'Copy the commercial invoice number from the supplier document. It must not belong to another order or shipment of this supplier.',invoiceDate:'Use the commercial invoice date printed on the supplier document.',realizedAmount:'Enter what the supplier actually received, in the displayed currency. Do not copy the amount sent unless they are confirmed equal.'};
function formSteps(modal){
 ...
 for(const field of invalid.slice(0,8))items.push(direct(field,'Next: '+label(field),field.type==='checkbox'?'Review the statement beside this checkbox. Check it only if you agree before submitting.':field.type==='file'?'Choose the required evidence for this form. Each attachment can be up to 50 MB; follow the displayed file-type rules.':(fieldHelp[field.name]||field.validationMessage||'Complete this required field.')+' Select Take me there to return to the field.'));
 if(!items.length){for(const f of fields.filter(f=>fieldHelp[f.name]).slice(0,5))items.push(direct(f,'Check: '+label(f),fieldHelp[f.name]));}
...
function pageSteps(){
 ...
 const guided=resolve('.order-step-guide');if(guided){const items=[...guided.querySelectorAll('li')].map((li,i)=>direct(li,String(i+1)+'. '+guided.querySelector('h2').textContent,li.textContent+' Select Take me there to return to the page.'));const blocked=guided.querySelector('.order-step-blocker');if(blocked)items.unshift(direct(blocked,'This step needs another user',blocked.textContent));const go=resolve('main .head-actions .button.primary');if(go)items.push(direct(go,'Then: '+go.textContent.trim(),'After checking the points above, open this action and complete its form. Nothing is saved by the guide.'));return items.length?items:base;}
