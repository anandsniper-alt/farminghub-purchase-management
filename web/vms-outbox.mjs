...
export function createVmsOutbox(host){
 ...
 let syncing=false,installPrompt=null,lastRendered=null;
 ...
 const changed=()=>{const c=host.context(),signature=JSON.stringify([list(),!!installPrompt,c.state?.revision]);if(signature===lastRendered)return;if(c.ui.view==='vms'&&!c.ui.modal&&!c.pageDirty){lastRendered=signature;host.render();}};
 async function sync(){
 ...
 async function enqueue(commandType,payload,base,vendorName){
  ...
  if(commandType==='VMS_SAVE_PROFILE'&&list().some(x=>x.vendorId===payload.vendorId&&x.commandType===commandType))throw new Error('This vendor already has a pending edit. Review it in Pending Sync first.');
 ...
 lastRendered=JSON.stringify([list(),!!installPrompt,host.context().state?.revision]);
