...
export function applyDomesticOrderCommand(state,type,p,ctx,h){
 ...
 if(type==='DOMESTIC_PO_SAVE'){
  ...
  const data={vendorId:vendor.id,vendor:{id:vendor.id,code:vendor.code,name:vendor.name,address:vendor.address||'',contact:vendor.contact||''},bom:{id:bom.id,code:bom.code,name:bom.name,reference:softwareReference(state,'domesticBoms',bom.id),revision:bom.revision,partCount:bom.lines.length},assemblyQuantity:count,lines,totalMinor,currency:'INR',deliveryDate,deliveryAddress:text(p.deliveryAddress,'Delivery address'),paymentTerms:text(p.paymentTerms,'Payment terms'),taxTerms:text(p.taxTerms,'GST / tax terms'),freightTerms:text(p.freightTerms,'Freight terms'),notes:typeof p.notes==='string'?p.notes.slice(0,3000):'',updatedAt:now,updatedBy:user.name};
