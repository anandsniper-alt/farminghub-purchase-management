...
export function previewTrackingImport(state,rows){const normalized=normalizeTrackingRows(rows),counts=new Map();for(const {normalized:n} of normalized)if(n.forwarderRef)counts.set(n.forwarderRef,(counts.get(n.forwarderRef)||0)+1);return normalized.map(x=>{const n=x.normalized,errors=[],warnings=[];if(!n.forwarderRef)errors.push('Forwarder Ref is required for automatic matching');if(counts.get(n.forwarderRef)>1)errors.push('Forwarder Ref is repeated in this file; keep one row per Ref');const matches=state.orders.flatMap(o=>o.shipments.map(s=>({o,s}))).filter(({s})=>s.status!=='CANCELLED'&&k(normalizeForwarderRef(s.forwarderRef))===k(n.forwarderRef));if(n.forwarderRef&&matches.length===0)warnings.push('No current shipment matches this forwarder Ref');const grouped=matches.length>1&&matches.every(x=>x.s.roGroupId&&x.s.roGroupId===matches[0].s.roGroupId);if(matches.length>1&&!grouped)errors.push('Forwarder Ref matches more than one current shipment');if(matches.some(x=>x.o.deletedAt))errors.push('A matched PO is deleted; restore it before importing updates');if(grouped)warnings.push('Updates '+matches.length+' linked PO shipments in this RO');const m=matches[0];if(m?.o.deletedAt)errors.push('Matched PO is deleted; an administrator must restore it before importing updates');if(m&&n.blNo&&m.s.blNumber&&norm(n.blNo)!==norm(m.s.blNumber))warnings.push('BL number differs from the shipment record');if(m&&n.supplier&&state.vendors.find(v=>v.id===m.o.vendorId)?.name&&!norm(n.supplier).includes(norm(state.vendors.find(v=>v.id===m.o.vendorId)?.name)))warnings.push('Supplier text differs; Ref remains the match key');let change='UNMATCHED';if(m){const diffs=[];if(n.etd&&n.etd!==m.s.etd)diffs.push(`ETD ${m.s.etd||'—'} → ${n.etd}`);if(n.eta&&n.eta!==m.s.eta)diffs.push(`ETA ${m.s.eta||'—'} → ${n.eta}`);if(n.blNo&&n.blNo!==m.s.blNumber)diffs.push('BL number update');if(n.statusText&&n.statusText!==m.s.lastForwarderStatus)diffs.push('Status update');change=diffs.length?diffs.join('; '):'NO CHANGE';}
 return {...x,matches:matches.map(({o,s})=>({orderId:o.id,orderNumber:o.number,shipmentId:s.id,shipmentNumber:s.number})),match:m?{orderId:m.o.id,orderNumber:m.o.number,shipmentId:m.s.id,shipmentNumber:m.s.number}:null,result:errors.length?'REJECTED':m?'MATCHED':'UNMATCHED',errors,warnings,change};});}
...
export function normalizeRateRows(rows){
 ...
 for(let i=0;i<rows.length;i++){const raw=rows[i],n={};for(const[h,v]of Object.entries(raw)){const a=aliases[norm(h)];if(a)n[a]=t(v);}if(!Object.values(n).some(Boolean))continue;const p=routeParts(n.loading);n.origin=p.origin;n.via=p.via;n.destination=k(n.discharge);n.containerType=k(n.volume);const rm=t(n.rateText).match(/^(?:(?:USD|\$)\s*)?((?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?)(?:\s*USD)?$/i);const rate=rm?Number(rm[1].replaceAll(',','')):NaN;n.rateUsd=Number.isFinite(rate)&&rate>0?rate:null;n.agentChargeUsd=n.rateUsd?agentChargeForRate(n.rateUsd):null;n.benchmarkUsd=n.rateUsd?n.rateUsd+n.agentChargeUsd:null;n.routeKey=[n.origin,n.via?'VIA '+n.via:'',n.destination,n.containerType].filter(Boolean).join('|');n.sourceRow=i+2;out.push({raw,normalized:n});}return out;
}
export function previewRateImport(state,rows,weekCode){const normalized=normalizeRateRows(rows),counts=new Map();for(const {normalized:n} of normalized)counts.set(n.routeKey,(counts.get(n.routeKey)||0)+1);return normalized.map(x=>{const n=x.normalized,errors=[],warnings=[];if(counts.get(n.routeKey)>1)errors.push('Route and container are repeated in this file; keep one rate per route and container');if(!t(weekCode))errors.push('Rate week is required');if(!n.origin||!n.destination)errors.push('Loading and discharge ports are required');if(!n.containerType)errors.push('Volume / container type is required');if(!(n.rateUsd>0))errors.push('O/F USD must contain a positive USD rate');if(state.freightRates?.some(r=>r.weekCode===weekCode&&r.routeKey===n.routeKey))warnings.push('This route already has a rate for the selected week; commit will add a correction snapshot');return {...x,result:errors.length?'REJECTED':'READY',errors,warnings};});}
...
export function shippingDocReadiness(order,shipment){const types=['COMMERCIAL_INVOICE','PACKING_LIST'];return types.filter(type=>!order.documents.some(d=>d.type===type&&d.shipmentId===shipment.id&&d.status==='FINAL'&&(type!=='COMMERCIAL_INVOICE'||!!t(d.invoiceNumber))));}
...
/** Quantities remain on their original PO lines even when a physical RO is shared. */
export function pendingLoadingRows(orders){return orders.filter(o=>!o.deletedAt&&!o.manualClosed&&o.revisions?.length).flatMap(o=>o.lines.map(l=>{const shipments=o.shipments.filter(s=>s.status!=='CANCELLED'),sum=ss=>ss.flatMap(s=>s.lines).filter(x=>x.lineId===l.id).reduce((n,x)=>n+x.quantity,0),allocated=sum(shipments),departed=sum(shipments.filter(s=>s.actualDeparture));return {o,l,ordered:l.quantity,allocated,departed,unallocated:l.quantity-allocated,pending:l.quantity-departed};}));}
