...
**WF-024 publication — 2026-09-13:** Runtime f5d5514 is live and the one-time Admin activation is complete. All 13 Manager stages persist after reload. Ashok and Suresh already had the required active Manager role and LAE Import scope; no profile edit was needed. Existing purchase/production/shipping/payment stages and all retained QA records are unchanged. Configured future policy edits remain authoritative; no automatic expiry. Live publication checks are distinct from earlier six isolated full flows.

## WF-025 - Move the page guide away from controls

**Date:** 2026-09-13. **Module/workflow:** global page guidance. **Previous workflow:** open a page, reach a control beneath the fixed right-hand mascot, scroll where possible to uncover it. **Requested change/reason:** mascot movement must expose covered controls; user restricts movement to left/right corners.

**New workflow:** (1) drag the mascot horizontally, (2) release to snap to the nearer bottom corner, (3) use the uncovered page control, (4) click/tap the mascot for the existing guide. Keyboard users focus the mascot and choose Left/Right. The side survives reloads. **Steps added:** optional corner selection. **Steps removed:** none. **Modified:** launcher placement; dragging does not open the guide. **Status changes:** none. **Roles/users affected:** all users, including sign-in guidance. **Dependencies:** browser pointer events/local preference, native guide dialog, responsive CSS. **Calculations/reports/database/API:** no business impact or server writes. **UI:** same branded pill, two bottom positions. **Backward compatibility:** absent/invalid/unavailable preference defaults to right; cancellation restores the prior side. **Risks:** browser tap suppression immediately after dragging, accidental activation and layering; covered by targeted checks. **Final implementation:** local native and standalone build; no live deployment claimed. **Related decision:** DEC-031. Prior history retained.
