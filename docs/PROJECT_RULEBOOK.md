...
## GLOBAL RULE — Downloaded release backups (DEC-067 / WF-061)
Every live release requires a fresh consistent production snapshot downloaded and packaged as a verified recovery ZIP with matching running source/assets and recovery instructions. Keep the five newest verified managed ZIPs in the main checkout's ignored backups/releases/. Prune older managed copies only after the new ZIP passes checksum/archive/isolated-restore checks; failure blocks publication. Never include operational credentials, reset reference counters, or overwrite later live writes without reconciliation. See BACKUP_RESTORE_RUNBOOK.md. This does not authorize deletion of unrelated archives or replace deferred cloud backups.
...
## MODULE-SPECIFIC RULE — Domestic quotation comparison (DEC-071)
Use saved quotations for read-only supplier and old/new comparison. Across suppliers: latest date per item/supplier, last saved for a same-date tie; show that quotation's date/reference. A partial quote does not erase earlier rates for omitted items. Historical mode compares only two selected snapshots from one supplier in chronological order. Do not fill historical omissions with other quotations.
Prices are INR per stated UOM before GST/freight/other conditions. Missing is not zero. Explicit zero remains a quote; percentage from old zero is unavailable. Different UOMs are not subtracted/ranked. Mark lowest only among at least two comparable selected quotations, retaining ties. Compute exact paise delta and half-up two-decimal percentage using shared helpers. Comparing must not issue business mutations or reprice BOMs. Current master pictures/Used in BOM are contextual, not historical composition. See DOMESTIC_PRICE_COMPARISON_REPORT.md for the complete formula contract.


## MODULE-SPECIFIC RULE — Domestic BOM save feedback (DEC-072 / WF-066)
Keep the user-entered revision reason mandatory. A blocked save must identify the field beside Save and focus it while retaining the selected parts, quantities, rates and notes. Unknown quantities/prices may save as Draft; never fill guessed values or silently confirm completeness. After assembly creation, open its assigned record's parts editor. Cancelling parts edits retains the already-created empty Draft.


## MODULE-SPECIFIC RULE — Assembly supplier total comparison (DEC-073 / WF-067)
Compare identical parts and quantities from one current saved assembly revision. Reuse domesticBomTotals for line-rounded INR costs and domesticPriceDifference for signed deltas/percentages against the selected reference supplier. Use DEC-071 latest per-part quotation selection and show actual quote dates/references. Complete totals require explicitly confirmed nonempty composition, valid quantities and matching-UOM INR quotes for every part; never fill absent supplier prices from manual BOM costs. Display partial subtotals as incomplete and exclude them from ranking/differences. Lowest requires at least two complete selected suppliers, including ties. Search changes the breakdown only, never the total. Comparing is read-only, with existing Domestic scope/viewing rights; no supplier adoption, BOM revision or PO creation. See DOMESTIC_ASSEMBLY_COMPARISON_REPORT.md for precision, validation, zero handling and revision boundaries.


## MODULE-SPECIFIC RULE — Domestic purchase orders (DEC-074 / WF-068)
Create Domestic POs only from a confirmed, saved assembly BOM with known part quantities. Executives, Managers and Admins with Domestic access can save/edit drafts; only Managers/Admins issue or cancel, as confirmed by the user. Use permanent automatic FH-LAE-D-PO-n references; no manual number or reuse. Derive part descriptions, photographs, units and per-assembly quantities from the saved BOM, not client fields. Multiply by a positive whole assembly count; select a subset for one active Domestic vendor when needed. Quoted rates must match vendor/item/UOM/rate; manually agreed INR rates require the draft reason. Payment, GST/tax, freight, delivery date/address and revision reason are explicit inputs. Totals are parts-only INR before GST/freight; do not infer tax values. Issue requires explicit confirmation, unchanged BOM revision, active supplier/items and a nonpast delivery date. Freeze an issued snapshot; no issued-order edits or deletions. Cancel with reason and retain the original document, number and audit. Supplier communication is separate from issuing/printing. Existing Import order workflows remain separate.
...
## Vendor-type payment agreements — DEC-086 / DEC-087
**MODULE-SPECIFIC RULE:** supplier PI/shipment/BL milestone terms must never be used as logistics payment terms. Under DEC-087, enter logistics partner payment terms directly in the existing Supplier master Add/Edit vendor form when the vendor kind is LOGISTICS, using `logisticsPaymentTerms`. Payment Terms Master is supplier-only; do not require a logistics template or a separate Logistics section. Retain existing typed templates, legacy IDs and recorded data for history without a live migration. Master edits require Manager/Admin access and a reason; preserve revision/audit history and existing executive update-request access. Supplier milestones must total 100%; logistics agreement text does not create a supplier payment schedule. Master edits do not rewrite existing PO terms, issued snapshots or payments. Linked vendor type changes must not strand product/order/forwarder relationships. DEC-087 supersedes only DEC-086's logistics-template selection and master layout.

### Logistics payment terms dropdown — DEC-088 / WF-082, 2026-09-26
**MODULE-SPECIFIC RULE:** the LOGISTICS vendor form uses a labelled payment-terms dropdown on Supplier master, with Custom terms text available for an agreement outside the listed choices. Blank means not recorded; never preselect a commercial agreement. Existing arbitrary `logisticsPaymentTerms` text opens as Custom without changing it. Save the selected preset or custom agreement through the existing string field and vendor command. DEC-088 supersedes only DEC-087's unrestricted text-field presentation; direct Supplier master entry, supplier-only Payment Terms Master, permissions, reason, validation, revisions and audit remain. Choices describe agreements and do not calculate logistics payable dates or change existing PO/payment records. Local demo only; publication requires the established release gate.

### Preset logistics payment terms — DEC-089 / WF-083, 2026-09-26
**MODULE-SPECIFIC RULE:** remove Custom terms and its free-text input from the LOGISTICS dropdown. Keep the eight existing presets and blank/not recorded. Only an existing unlisted saved agreement may appear as a selected `Recorded: <existing text>` option, preserving its value during unrelated edits; it does not permit new custom entry. DEC-089 supersedes DEC-088's Custom-entry UI only. Continue using the existing `logisticsPaymentTerms` field, vendor commands and preservation rules; no shared-domain restriction, migration or payable automation is added.


**DEC-089 preset extension, 2026-09-26:** the approved dropdown now includes `90 days from logistics invoice` and `120 days from logistics invoice` (ten presets total). Blank, no Custom entry, existing-record preservation and descriptive-only behavior remain unchanged.


**Supplier template lifecycle — DEC-086 release review:** inactive templates remain readable in existing supplier defaults and historical PO snapshots. Preserve the current inactive vendor assignment during unrelated edits, but disallow new inactive assignments. New PO creation requires deliberate active-term selection when a saved vendor default is inactive/unavailable; never silently fall back to another agreement. Existing draft/issued snapshots and explicit custom PO terms remain supported.


**Verification clarification, 2026-09-26:** `SAVE_PAYMENT_TERMS` requires a reason. Existing vendor edits retain their before/after audit and Remarks field; this release does not add a mandatory reason to `SAVE_VENDOR`. Prior paragraphs referring generally to a mandatory vendor-edit reason should be read with this correction.

## Supplier fixed-amount advances — DEC-092 / WF-086
**MODULE-SPECIFIC RULE, LAE Import candidate; not deployed.** Supplier terms may contain Percentage, Fixed amount and a final Remaining balance. Amounts are agreed in master/PO, not separately defined during payment capture. Require explicit matching PO currency, positive two-decimal fixed amounts and nonnegative remainder. Preserve percentage-only rounding, approved snapshots, master versions, existing approval controls and payment evidence. Fixed shipment amounts are apportioned across shipments, never repeated in full. Master changes do not alter existing POs; material typed-obligation revisions require active-payment reconciliation. See VALUE_PAYMENT_TERMS.md. DEC-090/091 Manual trial is superseded.

## Combined RO and correction rules — DEC-093 / WF-087
**MODULE-SPECIFIC RULE — LAE Import:** keep every selected line on its original PO/shipment. A shared RO group never combines supplier ledgers, PIs, payment authorizations or currencies. Enforce whole remaining quantities, approved PI, planning readiness, original-order permissions and atomic optimistic save. Only explicitly grouped active members may share a forwarder Ref; tracking must check access to every affected order. Cancelled quantities return to availability; pending departure includes reserved cargo. Keep per-member QC/evidence/arrival gates and allocate freight costs deliberately per member.
Voided-payment correction requires configured correction authority, reason, original supplier/order scope and fresh selected evidence, retains VOID history and links the replacement. Reject active duplicate bank references, duplicate active replacements and replacement allocations beyond remaining milestone amounts. A voided record does not reopen an obligation already covered by another active payment. Details and limitations: RO_LOADING_AND_PAYMENT_CORRECTIONS.md.
