...
### Vendor-type agreements — DEC-086
Hiding irrelevant inputs alone is insufficient: disable their submission and separate server-side term validation. Preserve historical values without presenting them as approved logistics agreements. Template changes must not mutate issued order/payment history. Native and standalone review modes use the same command and form logic.

### Direct logistics agreement entry — DEC-087
**Learning / scope:** MODULE-SPECIFIC vendor master UI, confirmed 2026-09-26. **Rule:** the user's final instruction places logistics partner payment terms directly in the existing Supplier master Add/Edit vendor form; supplier-only Payment Terms Master remains separate. **Reason / problem prevented:** a service agreement does not require a reusable logistics-template workflow or supplier milestone triggers. **Applies to:** LOGISTICS vendor entry/display through `logisticsPaymentTerms`, retaining existing typed templates, IDs and saved data for compatibility/history. **Does not apply to:** changing supplier milestone calculations, logistics payable automation, permissions, issued snapshots or a live data migration. DEC-087 supersedes the DEC-086 template-selection layout only. **Status:** implemented and browser-verified in native-server and standalone-review modes; not published.

### Preserve recorded agreements when adding dropdown choices — DEC-088
**Learning / scope:** MODULE-SPECIFIC Supplier master LOGISTICS form. The user's dropdown request changes entry format while retaining DEC-087's direct-entry location. Treat presets as choices requiring selection, never implicit commercial defaults; blank remains not recorded. Match known saved agreement strings to their preset and show other saved text as Custom unchanged. Continue saving `logisticsPaymentTerms` through existing vendor validation/revisions/audit, without interpreting invoice-day text as a payable calculation. Keep the separate Logistics master absent. DEC-088 supersedes DEC-087's text-field presentation only. Fresh native-server and standalone-review browser checks verified blank selection, Custom and 30-day preset save/reopen, type switching, mobile visibility and absent logistics template/master controls without JavaScript errors; review build passed. Local demo candidate, not published.

### Remove custom entry without losing recorded agreements — DEC-089
**Learning / scope:** MODULE-SPECIFIC Supplier master LOGISTICS form, 2026-09-26. **Rule / reason:** the user's explicit removal of Custom terms limits new UI choices to the existing presets and blank. Preserve any unlisted saved agreement using a selected `Recorded: <existing text>` option to prevent unrelated edits from erasing it. This supersedes DEC-088's Custom-entry presentation, without broadening new-entry choices. **Does not apply to:** domain validation, migrations, supplier terms or payable calculations. **Status:** implemented and locally browser-verified in native-server and standalone-review modes; review build passed. Not published.
